...
<main role="main" class="flex-shrink-0">
    <div class="container">
        ...
        <div class="rounded border p-5">
            ...
            <p>
            <form style="text-align: left" action="${pageContext.request.contextPath}/${cat}/editVehicle" method="post">
                <input type="hidden" value="${vehicle.id}" name="id">

                <div class="form-row">
                    <div class="form-group col-md-4">
                        <label for="vehicleCustomerId">Klient</label>
                        <select id="vehicleCustomerId" class="custom-select" aria-label="Imie i nazwisko klienta"
                            ...
                            <c:forEach items="${customerList}" var="customer">
                                <option value="${customer.id}"
                                        <c:if test="${vehicle.customerId!=null}">
                                            ...
                                            <c:if test="${customer.id != vehicle.customerId}">disabled</c:if>
                                        </c:if> >${customer.name} ${customer.surname}</option>
                            </c:forEach>
                        </select>
